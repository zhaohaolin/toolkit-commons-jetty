...
public class EmbbedJetty {
	...
	private void start() throws Exception {
		...
		if (webAppDir != null) {
			logger.info("load webPath=" + webAppDir);
		...
		server.start();
		logger.info("jetty embbed server started, port=" + port);
		...
		Runtime.getRuntime().addShutdownHook(new Thread() {
			...
			public void run() {
				try {
					...
					logger.log(Level.WARNING, "run jetty stop error!", e);
				}
			}
	...
	public static void main(String[] args) {
		...
		try {
			...
			logger.log(Level.WARNING, "Server Start Error: ", e);
			System.exit(-1);
		}
	}
